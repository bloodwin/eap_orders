...
function eap_order_ID($eap_order){
    echo $eap_order->order_id;
}
function eap_order_date($eap_order){
    echo $eap_order->created;
}
function eap_order_fio($eap_order){
    echo $eap_order->fio;
}
function eap_order_address($eap_order){
    echo $eap_order->address;
}
function eap_order_email($eap_order){
    echo $eap_order->email;
}
function eap_order_phone($eap_order){
    echo $eap_order->phone;
}
function eap_order_user_comment($eap_order){
    echo $eap_order->author_comment;
}
function eap_number_products($eap_order){
    echo $eap_order->items_count;
}
function eap_order_price($eap_order){
    $price = apply_filters('order_price',$eap_order->order_price,$eap_order);
    echo $price;
}
function eap_order_status($eap_order){
    echo eap_get_status_name_order($eap_order->order_status);
}
function eap_order_status_date($eap_order){
    echo $eap_order->order_status_date;
}
function eap_order_basket_full($eap_order){
    echo $eap_order->basket_full;
}

//Отображение свойств товара
function eap_product_ID($eap_product){
    echo $eap_product->product_ID;
}
function eap_product_title($eap_product){
    echo $eap_product->product_name;
}
function eap_product_price($eap_product){
    echo $eap_product->product_price;
}
function eap_product_number($eap_product){
    echo $eap_product->items_count;
}
function eap_product_summ($eap_product){
    echo $eap_product->summ_price;
}
function eap_product_permalink($eap_product){
...
function eap_get_order($eap_order_id){
    ...
    $eap_order = eap_setup_orderdata($eap_orderdata);
    return $eap_order;
}
...
function eap_get_chart_orders($eap_orders){
    ...
    foreach($eap_orders as $order){
        $eap_order = eap_get_order($order->id);
...
*/
    ...
    foreach($orderdata as $data){ 
        $eap_product = eap_setup_productdata($data);
