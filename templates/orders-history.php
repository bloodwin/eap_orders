...
<div class="order-data">
    <table>
        ...
        <?php foreach ($eap_orders as $eap_order) { ?>
            <tr>
                <td>
                    <a href="<?php echo rcl_format_url(get_author_posts_url($user_ID), 'eap_orders'); ?>&order-id=<?php $eap_order->getOrderId(); ?>">
                        <?php $eap_order->getOrderId(); ?>
                    </a>
                </td>
                <td><?php $eap_order->getCreated(); ?></td>
                <td><?php eap_order_status($eap_order->getStatus()); ?></td>
            </tr>
            <tr>
                <td colspan=3><?php $eap_order->getBasketString(); ?></td>
            </tr>
<?php } ?>
    </table>
</div>
